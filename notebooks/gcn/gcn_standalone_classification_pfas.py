...
if __name__ == "__main__":
    ...
    for i in range(3):
        ...
        y_hat_pfas = model.predict(pfas_test)

        df_results_pfas = pd.DataFrame({
            'smiles': pfas_test.flatten(),
            'predicted_epa': y_hat_pfas.flatten(),
            'actual_epa': pfas_epa.flatten()
        })

        # Save to a CSV file
        df_results_pfas.to_csv('../../data/replication_gcn/classification_gcn_{}_{}_PFAS.csv'.format(nseed, args.split), index=False)
