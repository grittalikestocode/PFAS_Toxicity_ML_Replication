...
# Dataset loader
def load_data(task_type, file):
    # Load the dataset
    ldtoxdb = pd.read_csv(file)
...
if __name__ == "__main__":
	...
	# Load data
    data_x, data_y, epa = load_data(task_type='regression', file='../../data/training_dataset.csv')
    ...
    val_df.to_csv('../../data/replication_gcn/regression_test_data.csv', index=False)

	# PFAS data for further test
    pfas_x, pfas_y, pfas_epa = load_data(task_type='regression', file='../../data/test_pfas_dataset.csv')
    test_pfas_x = pd.DataFrame(pfas_x, columns=['smiles']) 
    test_pfas_y = pd.DataFrame(pfas_y, columns=['actual_neglogld50'])
    test_pfas_epa = pd.DataFrame(pfas_epa, columns=['actual_epa'])
    ...
    for i in range(3):
        ...
        for fold_no, (train, test) in folds:           
            ...
            results.to_csv('../../data/benchmark-models/regression_{}_predictions_rep_{}_{}_test.csv'.format(sampling_type, nseed, fold_no))
        ...
        # Save to a CSV file
        df_results.to_csv('../../data/replication_gcn/regression_gcn_{}_{}_test.csv'.format(nseed, args.split), index=False)
    
        # On PFAS data
        y_hat_pfas = scaler.inverse_transform(model.predict(pfas_x))

        df_results_pfas = pd.DataFrame({
            'smiles': pfas_x.flatten(),
            'predicted_neglog': y_hat_pfas.flatten(),
            'actual_neglog': pfas_y.flatten(),
            'predicted_epa': converter.convert_to_epa(y_hat_pfas, pfas_x),
            'actual_epa': pfas_epa.flatten()
        })

        # Save to a CSV file
        df_results_pfas.to_csv('../../data/replication_gcn/regression_gcn_{}_{}_PFAS.csv'.format(nseed, args.split), index=False)
